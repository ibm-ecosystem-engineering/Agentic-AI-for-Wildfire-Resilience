...
from llm_agent.AgentCsvMain import AgentCsvMain
...
@apiLlmAgent.route('/api/agent/csv', methods=['POST'])
def invoke_csv():
    logger.debug("/api/agent/csv ...")

    payload = request.get_json()

    ### Call the main function to get the response
    agentCsvMain = AgentCsvMain()
    resp = agentCsvMain.invoke(payload)
...
def agent_research():
    return render_template('agent/agent_research.html')

# Route for the contact page
@apiLlmAgent.route('/agent_researchpaper')
def agent_researchpaper():
    return render_template('agent/agent_researchpaper.html')

# Route for the contact page
@apiLlmAgent.route('/agent_weather')
def agent_weather():
    return render_template('agent/agent_weather.html')


# Route for the contact page
@apiLlmAgent.route('/agent_csv')
def agent_csv():
    return render_template('agent/agent_csv.html')
